...
export class LeftPaneContactSection extends React.Component<Props, State> {
  ...
  private handleLnsLookup(lookupName: string, callback: any) {
    const lnsTimeout = window.CONSTANTS.LNS_DEFAULT_LOOKUP_TIMEOUT;
    ...
    this.setState({ loading: true }, async () => {
      const lnsMapping = await window.lokiSnodeAPI.getLnsMapping(
        lookupName,
        lnsTimeout
      );
  ...
  private async handleOnAddContact() {
    ...
    this.handleLnsLookup(recipientIDInput, (lnsMapping: any) => {
      ...
      if (lnsMapping.pubkey) {
        sessionID = lnsMapping.pubkey;
      }

      // Validate SessionID
      const error = lnsMapping.error
        ? Object.values(lnsMapping.error)[0]
        : validateNumber(sessionID, window.i18n);
